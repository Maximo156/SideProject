...
public class OtherAI : MonoBehaviour
{
    ...
    {
        //on = false;
        transform.position = new Vector3(transform.position.x, HeightNoise.getHeight(transform.position)[0] + WalkHeight, transform.position.z);
        if (requireTarget)
        {
    ...
    void AIFunctionality()
    {
        ...
        float minHeight = HeightNoise.getHeight(transform.position)[0] + WalkHeight;
    ...
    void WalkNewPath()
    {
        ...
        if (!walkInRandomDirection)
        {
            ...
            randomDirectionTimer = Time.time + Random.Range(0, 3);
        }
    ...
    void MoveTowards(Vector3 direction)
    {
        ...
        transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(direction + new Vector3(0.001f, 0, 0.001f)), rotationSpeed * Time.deltaTime);
    ...
    void MonitorGravity()
    {
        ...
        if ((!canFly)&&(floatHeight > 0.0f))
        {
            ...
            {
                RaycastHit floatCheck;
                Debug.DrawRay(transform.position, -Vector3.up* (floatHeight + 1.0f));
