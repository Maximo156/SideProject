Shader "Roystan/Grass"
{
    Properties
    {
		...
		_minDist("Min Distance", float) = 100
		_maxHeight("Max Height", float) = 100
		_minHeight("Max Height", float) = 100
    }
	...
	{
		float3 random = rand(IN[0].vertex.xyz);
		float3 pos = IN[0].vertex.xyz;
		pos.x = pos.x + random.x;
		pos.z = pos.z+random.z;
		...
		float dist = distance(_WorldSpaceCameraPos, pos);

		float2 textPos = mul(float2x2(0, 1, 1, 0), (IN[0].vertex.xz - playerPos + (0.5 * textWidth)) / textWidth);// ;
		//textPos = pos.xz * _WindDistortionMap_ST.xy + _WindDistortionMap_ST.zw + _WindFrequency * _Time.y;


		float biomeBlendNoise = tex2Dlod(biomeInfo, float4(textPos, 0, 0 )).x*2 - 1;


		int shouldCreateGrass = sign(worldPos.y- _minHeight) + sign(slope - _SlopeLimit) + sign(_maxDist - dist)  - sign(worldPos.y - _maxHeight) + sign(0.33 + biomeBlendNoise);

		

		//float biomeBlendNoise = GetNoise(pos.xz) * 0.5 + GetNoise(pos.xz / 100) * 0.5;
